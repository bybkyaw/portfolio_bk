...
const GhostText: React.FC = () => {
  ...
  const handleClick = (key: string) => {
    if (clickedLetters[key]) return; // Prevent retrigger while animating
    setClickedLetters((prev) => ({ ...prev, [key]: true }));

    setTimeout(() => {
      setClickedLetters((prev) => ({ ...prev, [key]: false }));
    }, 1000);
  };

  useEffect(() => {
    const timer = setTimeout(() => setSwitched(true), 7000);
    return () => clearTimeout(timer);
  }, []);

  const getLetterStyle = useMemo(() => {
    return (index: number) => {
    ...
    <section className={`${cinzel.className} ghost-section`}>
      <div className="ghost-container">
        <h2 className="ghost-line">
          {renderLine(switched ? firstLineNew : firstLineInitial, 'first')}
        </h2>
        <h2 className="ghost-line shifted">
          {renderLine(switched ? secondLineNew : secondLineInitial, 'second')}
        </h2>
      </div>
    </section>
  );
